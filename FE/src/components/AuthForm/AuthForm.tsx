...
const AuthForm = ({ isLogin }: Props) => {
  ...
  const onSubmit = async (data: IUser) => {
    ...
    toast.success("Đăng nhập thành công!");
	  ...
	  toast.error(error.response?.data?.message || "Đã xảy ra l��i!");
	}
  };
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mt-4">
          {isLogin ? (
            <Link to="/register" className="text-sm text-blue-500 hover:underline">Bạn chưa có tài khoản, chuyển sang đăng ký?</Link>
          ) : (
            <Link to="/login" className="text-sm text-blue-500 hover:underline">Bạn đã có tài khoản, chuyển sang đăng nhập?</Link>
          )}
        </div>
      </form>
    <ToastContainer/>
