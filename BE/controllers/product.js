...
export const getAllProduct = async (req, res) => {
    try {
        const product = await ProductModel.find({}).populate("categoryId").populate("sizeId").populate("colorId")
...
export const createProduct = async (req, res, next) => {
    try {
        ...
        const [updateCategory, updateSize, updateColor] = await Promise.all([
            CategoryModel.findByIdAndUpdate(product.categoryId, {
                $push: { products: product._id },
            }, { new: true }).catch(err => { console.error('Error updating category:', err); return null; }),
            
            SizeModel.findByIdAndUpdate(product.sizeId, {
                $push: { products: product._id },
            }, { new: true }).catch(err => { console.error('Error updating size:', err); return null; }),
            
            ColorModel.findByIdAndUpdate(product.colorId, {
                $push: { products: product._id },
